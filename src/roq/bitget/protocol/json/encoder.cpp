...
std::string_view Encoder::place_order(
  ...
  if (create_order.order_type == roq::OrderType::LIMIT) {
    auto time_in_force = [&]() -> protocol::json::TimeInForce {
      if (create_order.execution_instructions.has(ExecutionInstruction::PARTICIPATE_DO_NOT_INITIATE)) {
        return protocol::json::TimeInForce::POST_ONLY;
      }
      return map(create_order.time_in_force).template get<protocol::json::TimeInForce>();
    }();
    fmt::format_to(
        std::back_inserter(buffer),
        R"("price":"{}",)"
  ...
  fmt::format_to(
      ...
      R"("side":"{}",)"
      R"("orderType":"{}",)"sv,
      map(create_order.side).template get<protocol::json::Side>().as_raw_text(),
      map(create_order.order_type).template get<protocol::json::OrderType>().as_raw_text());
  if (create_order.position_effect != roq::PositionEffect{}) {
    fmt::format_to(
        std::back_inserter(buffer),
        R"("posSide":"{}",)"sv,
        map(create_order.position_effect, create_order.side).template get<protocol::json::PosSide>().as_raw_text());
  }
